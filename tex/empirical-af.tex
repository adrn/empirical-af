...
\todo{try Larry's approach to describing this instead}
...
In this, we take the stellar label $Y$ to be a smooth and differentiable function of the
phase space coordinates $Y = Y(\bs{w})$.
...
constant $Y$.
The expression above for the total time derivative (Equation~\ref{eq:tot-time-deriv}) is
equivalent to the collisionless Boltzmann equation (CBE; see, e.g.,
\citealt{Binney:2008}) thanks to Liouville's theorem: If the phase-space density is
constant along trajectories, then the stellar label $Y$ can be taken to be the local
phase-space probability density $Y = f$.
