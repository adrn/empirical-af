...
measurements of intrinsically-invariant stellar labels.
...
describe below.
Here we build on past work (CITE PW21) by developing a new framework for using ...
...
characteristics of stellar populations.
For example, \todo{age velocity relation, MAPs, etc.}.

Under the assumptions we adopted in Section~\ref{sec:vertical}, the correlations between
...
In the context of vertical dynamics, the vertical action is, by construction, constant
along an orbit, so any time-invariant function of the vertical action is also constant
along an orbit.
...
From the tracers, one cou;d trace the curves of constant stellar label $Y$ in the
vertical phase space $(z, v_z)$.
For any given invariant stellar label $Y$, the total time derivative $\Deriv{Y}{t}$ must
be equal to zero,
\begin{align}
...
\begin{align}
    a_z &= - v_z \, \pderiv{Y}{z} \left(\pderiv{Y}{v_z}\right)^{-1}
\end{align}
...
conditioned on action, $p(\bs{Y} \given J_z)$, such that the distribution of stellar
label values has properties or parameters that vary smoothly with the vertical action.

In practice, ...
However, this .... instead use moments of the ... like the mean abundance ...
\begin{equation}
