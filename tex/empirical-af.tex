...
\begin{align}
    p_z(z, v_z) &= \frac{1}{\sqrt{2\pi} \, s_z} e^{-\frac{E_z}{s_z^2}} \\
    E_z &= \frac{1}{2} v_z^2 + \Phi_z(z) \quad .
\end{align}
...
generate a set of
For all cases, when fitting to the simulated data we use a maximum Fourier expansion
order $\mmax = 2$ (so that $m={2, 4}$).
