...
% Hogg abstract
% The vertical positions and velocities of stars near the Sun --- the kinematics in the
% direction perpendicular to the disk plane --- can be used to measure the local
% dark-matter density and surface mass density, to constrain processes of scattering and
% heating, and to study out-of-equilibrium dynamics (e.g., with the \textsl{Gaia}
% phase-space spiral).
% With contemporary stellar surveys (e.g., \textsl{Gaia}, \textsl{APOGEE}, and others),
% the tracers of vertical dynamics are so numerous and so well measured that the orbit
% shapes are more-or-less directly visible in the data through studies of stellar density
% or element abundances.
...
% and force law can all be derived directly from that orbit foliation.
% We show that this flexible model does a good job of fitting orbits to stellar abundance
...
% and which also implicitly separates the vertical and radial dynamics.
...
\section{Vertical Dynamics in an Axisymmetric Disk} \label{sec:vertical}
...
distribution underlying a tracer population given a snapshot of kinematic data $\bs{w}$
for the tracers, where $\bs{w}$ represents the phase-space coordinates.
...
In particular, for now we will assume that the Galaxy is in equilibrium, that the DF is
...
In axisymmetric, equilibrium systems, generic orbits permit three integrals of motion
...
\end{equation}
where this integral is done over the full orbital path, which is set by the
gravitational field.
The top left panel of Figure~\ref{fig:sim-contours} shows 8 orbits with different values
of $J_z$ (but the same values of $J_R$ and $J_\phi$, $J_R=0$) computed in a Milky
Way-like gravitational potential. % TODO: define the potential maybe in appendix?
...
``labels'' $\bs{F}$, and that those labels are correlated with the orbital properties of
the tracers $\bs{F}(\bs{w})$, for now assuming the dependence is deterministic.
The labels can consist of any quantities that are close to being invariant over a star's
orbit and lifetime, such as surface element abundances, birth time, birth
position, or mass, to name a few.
% TODO: here mention "direct contouring" and that the phase-space density itself can be
% a label?
...
labels can only depend on the phase-space coordinates through the vertical action $J_z$
or any function of the vertical action and not the vertical angle $\theta_z$.
This comes from the assumption that the labels are invariant: If they depended on
orbital angle $\theta_z$, this would imply that the labels change over the course of an
orbit.
TODO: correlations can appear naturally... galaxy formation...
We can therefore use the labels as constants of motion themselves, which provides a
means to measure the orbital structure of the Galaxy directly, without having to assume
a form for the potential and without needing to compute the actions.
...
The orbital actions are extremely useful quantities for summarizing orbits and
...
In the previous section, we argued that in an equilibrium system with tracers tagged
with additional invariant labels that correlate with their orbits, the functional
dependence of the invariant labels must only be a function of the actions and not the
angles.
This means that we can use the labels as constants of motion themselves to trace out the
shapes of orbits in phase-space.
From the shapes of these orbits, we can then infer the underlying acceleration field and therefore constrain the mass distribution of the Galaxy.
In this section, we first discuss some subtleties of using stellar labels to trace orbits (Section~\ref{sec:oti-stat}), then ...


\subsection{Using moments of the stellar label distribution to map orbits}
\label{sec:oti-stat}

When we sketched out the conceptual basis for using invariant stellar labels to trace
orbits (Section~\ref{sec:vertical}), we assumed that the labels were deterministic
functions of the orbital actions.
We also implicitly assumed that we would have an infinite sample of stars to trace out
the curves of constant label value.
If this were true, our task would be straightforward: Identify curves of constant label value in the vertical phase-space,

With any finite sample size of tracers, we will never observe two or more stars on
\emph{exactly} the same orbit.
This conceptual explanation is additionally complicated by the fact that the galaxy
formation processes that introduce correlations between stellar labels and kinematics
also introduce scatter in the labels at fixed actions.
...
This means that at any location in phase-space, or at any value of the vertical action
$J_z$, there will be a distribution of stellar label values.
