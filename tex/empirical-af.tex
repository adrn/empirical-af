...
\todo{Audit abstract given current scope of the paper.}
The vertical positions and velocities of stars near the Sun can be used to measure the
local dark-matter density and surface mass density near the Galactic disk, to constrain
processes of scattering and heating, and to study out-of-equilibrium dynamics (e.g.,
...
We therefore hope that precise measurements of the dark matter within the Milky Way and
other nearby galaxies will enable new constraints on the particle nature of dark matter.
As a last example, the phase-space structure in the mass encodes, at some level, the
assembly history of the Galaxy \citep[e.g.,][]{Helmi:2018, Belokurov:2018}, and thus the
initial conditions of the primordial patch that evolved into our home in the Universe
\citep[e.g.,][]{Peebles:2011}.
...
For example, additional stellar labels can be added in to the phase-space distribution function (DF) explicitly to form an
``extended distribution function'' (eDF) $f(\bs{W}, \bs{Y})$
\citep[e.g.,][]{Sanders:2015, Binney:2023}, where here the vector $\bs{W}$ represents
the kinematic information (i.e. positions and velocities, or integrals of motion) and $\bs{Y}$ represents any additional stellar labels, like
element abundances.
This is a powerful approach because it allows for using the additional stellar labels to
help in the dynamical inference of the potential, but requires making choices
about how to parameterize the form of the eDF and any correlations between the kinematics
and stellar labels.
...
mono-abundance population approaches to modeling Milky Way disk kinematics.
...
That is, we assume that the gravitational potential is additively separable such that
\begin{equation}
...
phase space $p(Y \given z, v_z)$, as is apparent in Figure~\ref{fig:mgfe-zvz}.
...
To see this, we define the extended distribution function (eDF) $\mathcal{F}$ of the
vertical phase space as
\begin{equation}
    \mathcal{F}(z, v_z, Y, t) = f(z, v_z, t) \, p(Y \given z, v_z, t)  \label{eq:edf}
\end{equation}
where $f(z, v_z, t)$ is the phase-space density or distribution function (DF) and $p(Y
\given z, v_z, t)$ is the distribution of stellar labels at each location in phase space
and time \citep[see also][]{Sanders:2015}.
The DF necessarily satisfies the collisionless Boltzmann equation (CBE; see, e.g.,
\citealt{Binney:2008}),
\begin{align}
    \Deriv{f}{t} &= 0 \\
    0 &= \pderiv{f}{t} + v_z \, \pderiv{f}{z} - \deriv{\Phi}{z} \, \pderiv{f}{v_z}
        \label{eq:cbe}
        \quad .
\end{align}
Plugging the eDF into the CBE, we find that
\begin{align}
    \Deriv{\mathcal{F}}{t} &=
        \pderiv{\mathcal{F}}{t} +
        \pderiv{\mathcal{F}}{z} \, \deriv{z}{t} +
        \pderiv{\mathcal{F}}{v_z} \, \deriv{v_z}{t} +
        \pderiv{\mathcal{F}}{Y} \, \cancelto{0}{\deriv{Y}{t}} \label{eq:F-cbe}
\end{align}
where we assume that the stellar labels are time invariant so that $\deriv{Y}{t} = 0$.
If we multiply Equation~\ref{eq:F-cbe} by $Y$ and integrate over all $Y$ --- i.e.
compute the first moment of the eDF, $\meanY = \int \dd{Y} \, Y \mathcal{F}$
--- we get an expression in terms of the mean stellar label:
\begin{align}
    \Deriv{\meanYf}{t} &=
        \pderiv{\meanYf}{t} +
        \pderiv{\meanYf}{z} \, \deriv{z}{t} +
        \pderiv{\meanYf}{v_z} \, \deriv{v_z}{t} \label{eq:cbe-Yf} \\
    0 &= \cancelto{0}{\Deriv{f}{t}} +
        \pderiv{\meanY}{t} +
        \pderiv{\meanY}{z} \, \deriv{z}{t} +
        \pderiv{\meanY}{v_z} \, \deriv{v_z}{t}  \label{eq:cbe-Y}
\end{align}
where in moving from Equation~\ref{eq:cbe-Yf} to Equation~\ref{eq:cbe-Y}, we expanded
the derivatives and used the definition of the CBE to cancel the first term of
Equation~\ref{eq:cbe-Y}.
Rearranging Equation~\ref{eq:cbe-Y}, and dropping the time derivative of $\meanY$ under
our assumption of equilibrium, we can write the vertical acceleration as
\begin{align}
...
\end{center}
\caption{%
\todo{Did MCMC not work here? Where is error shaded region?}
...
% depending on vz?
% TODO: add text from chatting with Larry here.
...
realistic settings.
Our companion paper uses the OTI framework to measure the vertical acceleration profile
and the mass distribution of the Milky Way disk using \apogee\ data \citep{Horta:2023}.
\todo{Final thoughts...}
