...
The gray shaded region shows the 16th to 84th percentile acceleration trends.
...
\textrm{max}}$.
We use $m=\{2, 4, 6\}$ Fourier terms with 11, 5, and 5 spline knots, respectively.
...
We require that the coefficient function values at zero are zero so that
$e_2(0)=e_4(0)=e_6(0)=0$, and we assume that the sign of the functions are $(+,-,+)$ for
the $m=\{2, 4, 6\}$ coefficient functions, respectively.

We again measure the empirical scatter of the abundance data in each pixel of the
vertical phase space and use this (scaled by $1/\sqrt{N}$, where $N$ is the number of
particles in each pixel) as the uncertainty on the mean abundance value in each pixel.
We use the same regularization as in Section~\ref{sec:sim-sho} (L2 regularization on the
coefficient function parameters with a standard deviation of 0.1) for all Fourier
coefficient functions.
As above, we use a L-BFGS-B optimizer to minimize the regularized negative
log-likelihood.
This model has 30 parameters and optimizes in $\sim 140$~seconds on a single CPU.
...
The acceleration error at $z=1~\kpc$ is $\sim 3\%$ (in fractional error).
...
In this section, we apply the OTI framework to real data from the \apogee\ surveys and
\gaia\ Data Release 3.
...
\subsection{Modeling vertical stellar abundance gradients with \apogee\ data}
\label{sec:apogee}
