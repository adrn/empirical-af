...
In the vertical phase-space, the relevant action is the vertical action $J_z$ and its
...
% Note about how it's also important that the actions define surfaces that foliate
% phase-space?
...
size.
Figure~\ref{XX}, left shows the log-number density of this simulated sample in the
vertical phase-space.
We will fit the phase-space density of the simulated stars, hoping to recover a constant
frequency for all orbits and a vertical acceleration close to $a_z(z) =
-\omega^2\,z$.
...
elliptical radius (Equation~\ref{XX}).
For the Fourier coefficient function, we assert that the coefficient value must equal
zero at $\rzp = 0$, i.e. $e_2(0) = 0$, and that the coefficient value monotonically
increases with $\rzp$.
Requiring $e_2(0) = 0$ ensures that the inferred frequency at the midplane parameter
$\freqzero$ will correspond to the asymptotic orbital frequency as $\rzp \rightarrow 0$.
Assuming that the coefficient value increases with $\rzp$ (so that the orbit shapes can
only become more prolate with increasing $z_{\textrm{max}}$) is equivalent to assuming
that the volume density increases with increasing height $z$.
We use 5 spline knot locations $x_k^{(e_2)}$ equally spaced in $\rz$ between $0$ and
$r_{z, \textrm{max}}$, but we require the knot value at $\rzp = 0$, $y_0^{(e_2)}$, is
equal to zero $y_0^{(e_2)}=0$.

For this application, our parameters are summarized in Table~\ref{tbl:sho-params}
...
\begin{tabular}{c c c}
    ...
    $y_0^{\ln n}$ & Log phase-space density value at $r_z=0$ & TODO\\
    $y_{1\dots K}^{\ln n}$ & Log phase-space density knot values TODO & $(0)$\\
    $y_0^{e_2}$ & $m=2$ Fourier coefficient value at $r_z=0$ & fixed to $0$\\
    $y_{1\dots L}^{e_2}$ & $m=2$ Fourier coefficient knot values TODO & $(0)$\\
\end{tabular}
\caption{
    Sup.
