...
With the model described above, fitted to a given sample of data in a projection of
phase space, we can use the model to compute empirical values of the orbital actions,
angles, and frequencies.
The action and frequency are monotonic and smooth functions of the distorted radius \rz,
i.e. $J_z = J_z(\rz)$ and $\Omega_z = \Omega_z(\rz)$.
We can compute these dynamical quantities using the integrals in
...
\end{align}
That is, for a given phase-space coordinate $(z, v_z)^*$ at the current ellipsoidal
coordinates $(\rzp, \thzp)^*$, the integrals above are done along the curve of constant
$\rz^*$.
...
We then fit the simulated mean \abun{Mg}{Fe} abundance data using the OTI framework
...
objective function: We also measure the empirical scatter of the abundance values in
each pixel of the vertical phase space and adopt this as the uncertainty on the mean
...
We estimate uncertainties on the best-fit model parameters by using \jax\
