...
\section{Vertical Dynamics in an Axisymmetric Disk} \label{sec:dynreview}
...
\todo{Wait - is $a_R$ really the same as $\deriv{\Phi}{R}$?}
Under the assumptions stated above, the orbits in such a system are governed by a
gravitational potential $\bs{a}(R, z)$ that is related to the underlying mass density
through Poisson's equation,
\begin{equation}
    \frac{1}{R} \, \pderiv{}{R}\left(R \, a_R\right) + \pderiv{a_z}{z}
        = 4\pi \, G \, \rho(R, z) \quad .
\end{equation}
If we assume that we will always work in a small annular volume, the radial gradient in
the left hand side will be small and can be neglected, leaving only the vertical terms
\begin{equation}
    \pderiv{a_z}{z}|_R = 4\pi \, G \, \rho_z(z \,;\, R)
\end{equation}
where the expressions are implicitly done at some fixed radius $R$.
...
% TODO: change this figure to have 3 panels. Orbits, Mean stellar label, stellar label
% vs. Jz. Remove the phase-space density panels. Change color schemes.
% \begin{figure*}[t!]
% \begin{center}
...
The orbital actions $\bs{J}$ are extremely useful quantities for summarizing orbits and
...
We represent these quantities with the vector $\bs{Y}$, which we refer to as ``stellar
labels''.
In the Milky Way, these quantities are observed to correlate with the orbital
properties of stellar populations.
For example, this is apparent in the ... \todo{age velocity relation, MAPs, etc.}.
