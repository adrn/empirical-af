...
\newcommand{\freqzero}{\ensuremath{\Omega_{z,0}}}
...
\subsection{Orbital Torus Imaging with the Phase-space Density} \label{sec:oti-psd}
...
The core conceptual idea of OTI in this context is that the derivatives of the DF that
appear in the CBE (e.g., Equation~\ref{eq:cbe-1d}) only depend on the shapes of the
curves defined by level sets of the DF (i.e. curves of constant phase-space density).
...
take derivatives of the fitted function of the phase-space density to measure the
acceleration with the CBE.
This idea was referred to as ``direct contouring'' of the DF by \citet{Kuijken:XXXX},
but was ignored as being impractical at that time.
An interesting aspect of this approach is that the level sets of the DF correspond to
the orbits of individual stars.
In other words, the curves of constant phase-space density $f_z(z, v_z)$ (i.e. DF value)
correspond to orbits in this 1D, equilibrium setup. \todo{reference figure to
demonstrate this}
...
useful statement: For a higher-dimensional DF, if we could measure multiple independent
...
shapes will be close to ellipses with an aspect ratio that depends on the asymptotic
vertical frequency at the midplane, \freqzero.
That is, near the midplane, contours of the DF will correspond to curves of constant
elliptical radius $r_z'$, defined as
\begin{equation}
    r_z' = \sqrt{z^2 \, \freqzero + v_z^2 \, \freqzero^{-1}} \label{eq:rzp}
\end{equation}
for all values of the corresponding ellipsoidal angle $\theta_z'$,
\begin{equation}
    \tan \theta_z' = \frac{z}{v_z}\,\freqzero \label{eq:thetazp} \quad .
\end{equation}
Away from the midplane, the contours of the DF (and the shapes of orbits) have steadily
decreasing frequencies with increasing maximum height $z_{\textrm{max}}$, which leads
to, to first order, a changing aspect ratio of the contours.
...
To model more general contour shapes, we use a low-order Fourier expansion in the
ellipsoidal angle $\theta_z'$ to distort the ellipsoidal radius $r_z'$ into the
distorted ellipsoidal radius $r_z$, defined as
\begin{equation}
    r_z = r_z' \, \left[1 + \sum_m \epsilon_m(r_z') \, \cos{\left(m\,\theta_z'\right)}\right] \label{eq:rz}
\end{equation}
where we only consider even $m$ values to preserve the symmetry of the contour shapes.
To fit the phase-space density, we then use a model
