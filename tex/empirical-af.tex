...
comes from modeling stellar kinematics \citep[e.g.,][]{Binney:2008}.
...
kinematics we observe to the underlying mass distribution \citep{Kuijken:1991a, Binney:2008, Green:2022, TODO}.
...
the \gaia\ Mission (Gaia:2016, Gaia:2018, Gaia:2022), deep, multi-band photometric
surveys such as the Sloan Digital Sky Survey (SDSS; TODO) and Dark Energy Survey (DES;
TODO), and high-resolution spectroscopic surveys such as the Apache Point Observatory
Galactic Evolution Experiment (APOGEE; TODO).
...
properties, and precision of the available data for Milky Way stars (CITE; LAMOST,
GALAH, WEAVE, 4MOST, SDSS-V).
...
% generate N samples from the \df. We then sub-select a sample of star particles
...
We run the chains for 1000 initial samples to allow the NUTS sampler to tune the
hyperparameters of the sampling algorithm (i.e., the mass matrix), and then run for an
additional 1000 steps.
The middle panel of Figure~\ref{fig:sho-validation} shows the maximum a posteriori (MAP)
vertical acceleration profile, $a_z(z)$ (computed with Equation~\ref{eq:az-ugly}) as the
under-plotted black line, and the over-plotted, dashed (green) line shows the true
vertical acceleration relation, $a_z = -\omega^2 \, z$.
This demonstrates that for this toy dataset we are able to recover the true acceleration
profile using OTI.
In the same (middle) panel, we also plot a shaded (gray) polygon to represent the
acceleration profile span for the 16th--84th percentile values of the MCMC samples.
However, in this case, the uncertainty region is comparable to the thickness of the
black line.

The left panel of Figure~\ref{fig:sho-validation} shows eight orbits with equally-spaced
(but arbitrary) values of the vertical velocity at $z=0$: The under-plotted solid
(black) lines show the orbital trajectories inferred using the MAP parameter values with
our OTI model, and the over-plotted dashed (green) lines show the true orbital
trajectories for this toy model, demonstrating agreement.
...
Here, however, we use $m=\{2, 4\}$ Fourier terms with 8 and 4 spline knots,
respectively.
...
The middle panel of Figure~\ref{fig:qiso-validation} shows the inferred vertical
...
The left panel of Figure~\ref{fig:qiso-validation} shows eight orbits with
...
To set $v_y$, we take the true actions computed for the particles in our selected region
and fit a 3rd-order polynomial to the mean relationship between $J_\phi$ and $J_z$ in
this region (which arises primarily because of the finite selection on $R$).
We ...

for this toy model, demonstrating agreement.
...
As a demonstration that OTI is
...
We use a snapshot of the simulation after \todo{X} pericentric passages of the
...
actions is both axisymmetric and time-invariant. \todo{Jason: fix previous sentence if
not true}
