...
\title{
    ...
    Empirically Modeling the Vertical Phase-space of the Galaxy
}
...
\begin{abstract}\noindent
% Context
In a steady-state dynamical system, gradients of the phase-space density distribution of
any tracers can be simply related to gradients of the underlying gravitational potential
(i.e. the acceleration field).
...
the presence of weak disequilibrium that makes fewer assumptions about the form of the
mass distribution.
% Methods
As a step toward this goal, here we outline a flexible method for empirically modeling
the vertical phase-space density (or mean statistics of stellar invariants in this
phase-space, like element abundances) of stars in the Galactic disk.
...
of an unperturbed galaxy have predictable symmetries and can be represented as a
low-order Fourier expansion away from an ellipse.
% Results
...
cases OTI recovers the local acceleration field (\todo{or frequency structure, or mean
density, or whatever we actually learn!}).
We then show that this method enables us to compute orbital actions, angles, and
frequencies for stars where the approximation of a separable 1D phase-space is valid.
We finally demonstrate that this method provides a useful tool for studying the
\emph{residuals} away from an equilibrium model, such as the ``\gaia\ Phase Spiral.''
% Conclusions
Can add perturbations, hoping to go to 4D phase-space next. Some conclusions.
...
Our goal is to use the stellar phase-space density $f(\boldsymbol{x}, \boldsymbol{v})$
(or statistics computed from stellar invariants for stars on different orbits
$X(\boldsymbol{x}, \boldsymbol{v})$) to infer a local transformation to actions, angles,
