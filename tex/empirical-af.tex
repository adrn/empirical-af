...
\begin{equation}
    r_z' = \sqrt{(z-z_0)^2 \, \freqzero + (v_z-v_{z,0})^2 \, \freqzero^{-1}} \label{eq:rzp}
\end{equation}
for all values of the corresponding ellipsoidal angle $\theta_z'$,
\begin{equation}
    \tan \left(\theta_z'\right) = \left(\frac{z-z_0}{v_z-v_{z,0}}\right) \, \freqzero
    \label{eq:thetazp}
\end{equation}
...
expansion in the ellipsoidal angle $\theta_z'$ to distort the ellipsoidal radius $r_z'$
into the distorted ellipsoidal radius $r_z$, defined as
\begin{equation}
...
The functions $\epsilon_m(r_z')$ describe the radius-dependent amplitude of the Fourier
terms, which we require to all be zero at $r_z'=0$, and the values are typically $\ll
1$ within a few disk scale heights.
This expansion is motivated by the fact that we expect to need just a few Fourier terms:
an $m=2$ distortion with an amplitude that varies with $r_z'$ will change the aspect
ratio of the elliptical contours as a function of $r_z'$, which acts like a changing
...
In applications below, we try different maximum $m$ values, $m_{\textrm{max}}$, beyond
$m=4$.
\todo{Could show plots of rzp vs thetazp with different Fourier mmax fits?}

The distorted radius $r_z$ (Equation~\ref{eq:rz}) is effectively a label for the orbits
and therefore also for contours of the DF.
To fit the phase-space density with the distorted radius $r_z$, we then need to specify
a functional form for the dependence of the phase-space density on $r_z$, $f_z(r_z)$.
The model components that must be specified to fit the phase-space density are then: the
Fourier distortion amplitude functions $\epsilon_m(r_z')$ for all $m$ orders up to the
specified $m_{\textrm{max}}$, the functional form of the phase-space density $f_z(r_z)$,
and the zero-point position and velocity $z_0$ and $v_{z,0}$.
...
TODO: how to go from this to constraints on acceleration / density
...
$\theta_z$ at that point can be computed as the fraction of an orbital period traversed
up to that point,
\begin{equation}
    \theta_z = \frac{2\pi}{T_z} \, \int_0^{z} \frac{\dd z'}{v_z(z' \,;\, r_z(z^*, v_z^*))} \quad ,
\end{equation}
where $z'$ is an integration variable, and the function $v_z(z' \,;\, r_z(z^*, v_z^*))$
indicates that this is a curve at constant $r_z$ as determined by the model parameters
